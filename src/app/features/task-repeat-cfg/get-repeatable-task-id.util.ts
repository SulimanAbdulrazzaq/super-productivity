...
/**
 * Inverse of getRepeatableTaskId: extracts the repeat config id and due day
 * from a deterministic instance id.
 *
 * Used to recognize notifications that were pre-scheduled for a predicted
 * future instance (see mobile-notification.effects.ts #7850) when the task
 * entity does not exist yet.
 *
 * @param taskId - The task id to inspect
 * @returns The repeat config id and due day, or null if the id was not
 * generated by getRepeatableTaskId.
 */
export const parseRepeatableTaskId = (
  taskId: string,
): { repeatCfgId: string; dueDay: string } | null => {
  const match = /^rpt_(.+)_(\d{4}-\d{2}-\d{2})$/.exec(taskId);
  if (!match || !isDBDateStr(match[2])) {
    return null;
  }
  return { repeatCfgId: match[1], dueDay: match[2] };
};
