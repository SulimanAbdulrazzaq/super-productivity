...
- **Permissions** (all off by default):
  ...
  - _Add new tasks to the Inbox_ — `create_task` only. The title and notes are stored exactly as sent: no short syntax is parsed and nothing from the currently open view (project, tag, Today) is applied. It reveals no existing data.
